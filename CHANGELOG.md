...
The repokit modernization (ADR-1/2/3): repokit stops driving Claude as a subprocess and becomes **equipment Claude loads** — a generated plugin (MCP + native subagent personas + hooks + the spec-driven workflow runner). Authored as the foundational set directly (ADR-2 inception clause), then dogfooded through its own gated workflow.
...
- **Agent (init)**: `./repo init` now drives config-surface generation — after bootstrapping the venv it invokes the generation layer (via a shared `generate_surface` helper, also behind `./repo generate`) to emit the agent-config surface, honoring the adoption guard (a pre-existing un-managed file is warned, not clobbered or overwritten). Completes the `init-mission-shift` (init from venv-bootstrap → config-surface generation).
...
- **Tests**: Add `test_worktree.py` — 23 tests covering worktree bootstrap, lifecycle (`ensure`/`remove`), prompt template validation (no stale placeholders, MCP tool references), and `_render_role_prompt` without legacy kwargs.
